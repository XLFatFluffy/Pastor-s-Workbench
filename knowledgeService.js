...
function normalizeSource(input = {}) {
  ...
  const source = {
    id: clean(input.id) || uid('knowledge'),
    title,
...
async function embedTexts(texts) {
  ...
  if (!Array.isArray(vectors) || vectors.length !== texts.length) {
    throw new Error(`Ollama did not return embeddings. Make sure the model is pulled: ollama pull ${EMBED_MODEL}`);
  }
  return vectors;
}
...
/**
 * Add a knowledge source (book/text file or raw text) to the local semantic
 * knowledge store: extracts text, chunks it, embeds each chunk with
 * nomic-embed-text via Ollama, and persists source + chunk records.
 * Accepts either a File/Blob (via `file`) or raw text (via `text`).
 */
export async function addKnowledgeSource({ file = null, text = '', title = '', author = '', description = '' } = {}, onProgress) {
  ...
  const source = normalizeSource({
    ...
    author, description, filename, mime_type: mimeType, file_size: fileSize,
    chunk_count: chunks.length, status: 'embedding'
  });
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i += BATCH) {
      ...
      batch.forEach((chunk, j) => {
        records.push({
          id: `${source.id}:chunk:${chunk.index}`,
          source_id: source.id,
          index: chunk.index,
...
export async function listKnowledgeSources(search = '') {
  ...
  return (await all(SOURCE_STORE))
    .filter(s => !needle || [s.title, s.author, s.description, s.filename].join(' ').toLowerCase().includes(needle))
...
export async function deleteKnowledgeSource(id) {
  const chunks = (await all(CHUNK_STORE)).filter(c => c.source_id === id);
  for (const chunk of chunks) await remove(CHUNK_STORE, chunk.id);
  await remove(SOURCE_STORE, id);
}

export async function getKnowledgeStats() {
  const sources = await all(SOURCE_STORE);
  const chunks = await all(CHUNK_STORE);
  return { sources: sources.length, chunks: chunks.length, characters: chunks.reduce((n, c) => n + String(c.content || '').length, 0) };
}

/** Semantic search over the knowledge store — cosine similarity against the query embedding. */
export async function searchKnowledge(query, { limit = 6, sourceIds = [] } = {}) {
  const needle = clean(query);
  if (!needle) return [];
  const chunks = (await all(CHUNK_STORE)).filter(c => !sourceIds.length || sourceIds.includes(c.source_id));
  if (!chunks.length) return [];
  const [queryVector] = await embedTexts([needle]);
  const sources = new Map((await all(SOURCE_STORE)).map(s => [s.id, s]));
  return chunks
    .map(c => ({ ...c, score: cosineSimilarity(queryVector, c.embedding || []) }))
    .sort((a, b) => b.score - a.score)
    .slice(0, limit)
    .map(r => ({ ...r, source: sources.get(r.source_id) || null, citation: `${sources.get(r.source_id)?.title || 'Source'}${sources.get(r.source_id)?.author ? ` — ${sources.get(r.source_id).author}` : ''}, chunk ${r.index + 1}` }));
}

export async function buildKnowledgeContext(query, options = {}) {
  try {
    return await searchKnowledge(query, options);
  } catch {
    // Embedding model unavailable (e.g. not pulled, or running in the browser dev build) — degrade quietly.
    return [];
  }
}

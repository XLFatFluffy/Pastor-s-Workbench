...
export async function render(mount) {
  mount.innerHTML = `
    ...
    <section class="study-grid ai-workspace">
      <div class="reader-panel"><div class="reader-panel__head"><div><p class="canvas__eyebrow">Conversation</p><h2>Ask your Workbench</h2></div><span id="ai-status" class="pill">Checking Ollama…</span></div>
        ...
        <form id="ai-form" class="tool-form"><label>Question or instruction<textarea id="ai-input" rows="5" required placeholder="Ask about a passage, sermon, doctrine, book, or research question…"></textarea></label><div id="ai-attachments" class="ai-attachments"></div><div class="ai-toolbar"><label class="ai-check"><input id="ai-books" type="checkbox" checked> Use my Book Library</label><label class="button" for="ai-attach-input" style="cursor:pointer">Attach file</label><input id="ai-attach-input" type="file" accept=".pdf,.txt,.md,.markdown,.html,.htm,.json,.csv" hidden><label>Model<select id="ai-model"></select></label><button class="button button--primary button--large" type="submit">Ask AI</button></div><div id="ai-error" class="error-panel" hidden></div></form>
      </div>
  ...
  const attachmentsEl = mount.querySelector('#ai-attachments');
  const renderAttachments = () => {
    attachmentsEl.innerHTML = pendingAttachments.map((a, i) => `<span class="pill">${esc(a.name)} <button type="button" data-remove-attachment="${i}" aria-label="Remove attachment">×</button></span>`).join('');
    attachmentsEl.querySelectorAll('[data-remove-attachment]').forEach(btn => btn.onclick = () => { pendingAttachments.splice(Number(btn.dataset.removeAttachment), 1); renderAttachments(); });
  };
  mount.querySelector('#ai-attach-input').onchange = async e => {
    const file = e.target.files?.[0]; e.target.value = '';
    if (!file) return;
    const err = mount.querySelector('#ai-error');
    try { pendingAttachments.push(await readAttachmentFile(file)); renderAttachments(); }
  ...
  mount.querySelector('#ai-form').onsubmit = async e => {
    ...
    const attachmentNote = pendingAttachments.length ? ` <small class="muted">(${pendingAttachments.map(a=>esc(a.name)).join(', ')})</small>` : '';
    messages.insertAdjacentHTML('beforeend', `<article class="ai-message ai-message--user"><strong>You</strong><p>${esc(prompt)}${attachmentNote}</p></article><article id="ai-working" class="ai-message"><strong>Workbench AI</strong><p>Thinking…</p></article>`);
    input.value='';
    const attachmentsForRequest = pendingAttachments;
    pendingAttachments = []; renderAttachments();
    try {
      const out = await askAI({ message: prompt, history, includeBooks: mount.querySelector('#ai-books').checked, attachments: attachmentsForRequest, model: modelSelect.value });
      ...
      const allRetrieved = [...out.context, ...(out.knowledgeContext || [])];
      sources.innerHTML = allRetrieved.length ? allRetrieved.map((r)=>`<article class="knowledge-card"><div class="knowledge-card__head"><div><h3>${esc(r.book?.title || r.source?.title || 'Source')}</h3><small>${esc(r.citation)}</small></div><button class="text-button" data-copy-source="${esc(r.id)}">Copy passage</button></div><p>${esc(r.content)}</p></article>`).join('') : '<div class="empty-state">No passages were retrieved for this answer.</div>';
      sources.querySelectorAll('[data-copy-source]').forEach(b=>b.onclick=async()=>{const r=allRetrieved.find(x=>x.id===b.dataset.copySource);if(r){await navigator.clipboard.writeText(`${r.citation}\n\n${r.content}`);b.textContent='Copied';}});
